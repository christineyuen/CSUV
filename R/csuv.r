...
get.csuv.mod<-function(X, Y, intercept,
  ...
  # csuv.result$PR.mod.collection = get.PR.mod.collection(X, Y, intercept, all.fits$flds,
  #                                                    csuv.result$mod.collection,
  #                                                    csuv.result$variable.freq,
  #                                                    num.core = num.core)
...
get.csuv.final.mod<-function(X, Y, intercept, unique.fit,
  ...
  csuv.m = csuv.thr.by.freq(X, Y, intercept,
                          ...
                          freq.thr = 0.5,
                          coef.est.method)
  csuv.ebic = csuv.by.ebic(X, Y, intercept, var.order = var.sel.freq.n.order$var.order, size.thr = sum(var.sel.freq.n.order$var.sel.freq>=0.1) )
