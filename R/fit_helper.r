...
# lm.compare.method<-function(X, Y, intercept, method.names = NULL){
#   compare.methods = get.compare.methods()
#   if(!is.null(method.names)){
#     compare.methods = compare.methods[method.names]
#   }
#   result = do.call(rbind, lapply(compare.methods,
#                                  function(method) method(X = X, Y = Y, intercept = intercept)$est.b))
#   rownames(result) = names(compare.methods)
#   return (result)
# }
